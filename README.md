...
Отключаем уже настроенный *VPN-шлюз* и беремся за настройку *роутера*. Описывать ее с нуля не вижу смысла - вам потребуется минимально рабочая конфигурация, а уже поверх нее настроим остальное.

### 2.1. Создаем интерфейс bridge-awg и добавляем в него ether4
...
### 2.10 Подключаем VPN-шлюз к роутеру

Подключаем уже настроенный *VPN-шлюз* к *роутеру* согласно схеме в начале инструкции:
- ether5 (RouterOS) -> wan (OpenWrt)
- ether4 (RouterOS) -> lan (OpenWrt)

### 2.11. Проверяем маршрутизацию с клиентского ПК
